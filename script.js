...
const playMusic = (track) => {
    ...
    currentsong.src = `${process.env.PUBLIC_URL}/songs/${currentfolder}/` + track;
    currentsong.play();
...
async function getsongs(folder) {
    ...
    let a = await fetch(`${process.env.PUBLIC_URL}/songs/${folder}/`);
    ...
    for (let index = 1; index < s.length; index++) {
        ...
        if (element.href.endsWith(".mp3")) {
            songs.push(element.href.split(`${process.env.PUBLIC_URL}/songs/${folder}/`)[1])
        }
    }
